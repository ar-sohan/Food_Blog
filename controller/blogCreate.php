<?php
    ...
    if(!isset($_POST['submit'])){
        header('location: ../view/blogCreate.php');
        exit;
    }
    ...
    if($newId > 0){
        $_SESSION['flash'] = ['type'=>'success', 'msg'=>'Blog published.'];
        header('location: ../view/blog.php');
    } else {
        $_SESSION['flash'] = ['type'=>'error', 'msg'=>'Could not publish blog. Please try again.'];
        $_SESSION['old'] = [
            'title' => $title,
            'content' => $content,
            'post_type' => $postType,
